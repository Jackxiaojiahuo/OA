...
if(s=="invalid login name or password") return "用户名不存在或者口令错误，请重新输入";
...
if(s=="starting workflow test ...") return "正在启动工作流仿真测试 ...";
...
function $form2word() {
  ...
  for(var i=1; i<fds.Count; i++) {
    ...
    if(!ele) opener.document.getElementById(fds.Item(i).Name);
    if(ele) {
