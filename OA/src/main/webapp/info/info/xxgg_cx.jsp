...
<script>
	$(function() {
		...
		//var co={"names":s,"count":c}
			location.href='../../xxgg.do?action=cx&names='+s+'&count='+c+'';
		});
	})
	function sub(num){
		var s=$('input[name=field]:checked()').val();<!--是单选按钮是标题或发信人还是收件人-->
		var c=$('input[name=con]').val();<!--记入输入的信息-->
		if(num==1){
			location.href='../../xxgg.do?action=cx&names='+s+'&count='+c+'&index=1';
		}else if(num==2){
			location.href='../../xxgg.do?action=cx&names='+s+'&count='+c+'&index=${page.index-1 eq 0 ? 1 : page.index-1}';
		}else if(num==3){
			location.href='../../xxgg.do?action=cx&names='+s+'&count='+c+'&index=${page.index+1 gt page.total ? page.total : page.index+1}';
		}else if(num==4){
			location.href='../../xxgg.do?action=cx&names='+s+'&count='+c+'&index=${page.total}';
		}
	}
</script>
</head>
      ...
      <td id="colloaForm"><table cellspacing="0" cellpadding="0" border="0">
          <tbody>
            <tr>
              ...
              <td align="right"><script language="javaScript">/* Code removed by ScrapBook */</script><a href="javascript:location.reload()" class="button1 button1L" title="刷新"><i class="fa fa-bolt fa-lg"></i></a><span id="oWorkflowList1"></span><a href="../../xxgg.do?action=ss" class="button1 button1R" title="关闭"><i class="fa fa-times-circle fa-lg"></i></a></td>
            </tr>
          </tbody>
        </table>
        <br>
        <table class="tableList" cellspacing="0" cellpadding="0" border="0">
         ...
         <!--jsp页面信息  ${param.value} -->
          <c:forEach var="n" items="${ncx }">
          				<tr class="">
			              <td><a href="../../xxgg.do?action=ss&id=${n.noticeId }">
